 ...
 */
static void
fetchImage(GnLink& link, gnsdk_link_content_type_t contentType)
{
	GnImagePreferenceType imgPreference = kImagePreferenceSmallest;
	gnsdk_cstr_t          fileName      = GNSDK_NULL;
	GnImageSize           imageSize     = kImageSize170;
	gnsdk_cstr_t          imageTypeStr  = "";
...
queryForAlbumImages(GnUser& user)
{
	...
	/* Below demonstrates fetching multiple pieces of content from the same GnLink object.
	** Alternatively each fetch can use a new GnLink, but reusing the object can be more efficient.
	*/
	fetchImage(link, gnsdk_link_content_cover_art);
	fetchImage(link, gnsdk_link_content_image_artist);
