...
#define SAMPLE_LOOKUPDATABASE_ID			"sample_db_id"
#define SAMPLE_LOOKUPDATABASE_LOCATION		"../../../../sample_data/gdb"
...
public:
    ...
    MusicIdStreamIdentifyingStatusEvent(GnMusicIdStreamIdentifyingStatus status, IGnCancellable& canceller)
    {
        ...
        {
            std::cout << std::endl << " identify ended " << std::endl;
        }
    }
...
/*-----------------------------------------------------------------------------
 *  DisplayLocalDatabaseInfo
 *    Display local Gracenote DB information.
 */
void
DisplayLocalDatabaseInfo( GnLookupDatabase &lookupDatabase )
{
	GnDataObject info = lookupDatabase.Info();

	GnRenderOptions renderOptions;
	GnString infoXml = info.Render( renderOptions.Xml() );

	/* To see detailed information about the Lookup Database, enable this:
	std::cout << infoXml << std::endl;
	*/
}
...
EnableLocalLookups()
{
	...
	config.Set(kConfigOptionLocationLookupDatabaseAll, SAMPLE_LOOKUPDATABASE_LOCATION);
	config.Set(kConfigOptionEnableLookupDatabaseMusicID, GNSDK_VALUE_TRUE);

	GnLookupDatabase lookupDatabase(SAMPLE_LOOKUPDATABASE_ID, config);

	DisplayLocalDatabaseInfo( lookupDatabase );

	/* The database will be closed automatically at shutdown, but if your app
	wants to close it sooner, there is a GnLookupDatabase::Close() API. */

	/* Enable Local Stream module to use for local queries */
     ...
     ** database locations:
     */
	gnLookupLocalStream.StorageLocation(SAMPLE_LOOKUPDATABASE_LOCATION);
}
...
LoadLocale( GnManager& gnMgr, GnUser& user )
{
	...
	gnMgr.LocalesStore().Location(SAMPLE_LOOKUPDATABASE_LOCATION);
...
main(int argc, char* argv[])
{
	...
	try
	{
		...
		GnLog sampleLog(
			...
			GnLogFilters().All(),			/* Include only error and warning entries */
