 ...
 */
static bool
fetch_image(GnLink& link, GnLinkContentType contentType, const char* imageTypeStr)
{
	...
	try
	{
		...
		case kLinkContentCoverArt:
			linkContent = link.CoverArt(imageSize, kImagePreferenceExact);
			break;

		case kLinkContentImageArtist:
			linkContent = link.ArtistImage(imageSize, kImagePreferenceExact);
			break;

		case kLinkContentGenreArt:
			linkContent = link.GenreArt(imageSize, kImagePreferenceExact);
			break;
...
perform_image_fetch(GnDataMatch& dataMatch, GnUser& user)
{
	...
	if (dataMatch.IsAlbum())
	{
		...
		if (fetch_image(link, kLinkContentCoverArt, "cover art"))
		{
			/* if no cover art, try to get the album's artist image */
			if (fetch_image(link, kLinkContentImageArtist, "artist image"))
			{
				/* if no artist image, try to get the album's genre image so we have something to display */
				fetch_image(link, kLinkContentGenreArt, "genre image");
			}
		}
	}
	...
	)
{
	...
	if (count > 0 )  
	{   
		got_match = 1;   /* OK, we got at least one match. */
...
EnableStorage(GnLookupMode lookupMode)
{
	...
	*/
	...
	{
		storageSqlite.StorageLocation("../../../../sample_data/gdb");
	}
}
...
EnableLocalLookups()
{
	...
	** database locations:
	*/
	gnLookupLocal.StorageLocation(kLocalStorageAll, "../../../../sample_data/gdb");
