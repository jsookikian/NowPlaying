...
doMoodgridRecommendations(
	GnUser&									user, 
	moodgrid::GnMoodgridProvider&			provider,  
	playlist::GnPlaylistCollection&			collection, 
	moodgrid::GnMoodgridPresentationType	type
)
{
...
{
	gnMgr.LocalesStore().Location("../../../../sample_data/gdb");
...
main(int argc, char* argv[])
{
	...
	try
	{
		...
		for (moodgrid::moodprovider_iterator itr = providers.begin(); itr != providers.end(); ++itr)
		{
			...
			doMoodgridRecommendations(user, *itr,  myCollection, moodgrid::kMoodgridPresentationType5x5);

			/* create, query and print a 10 x 10 moodgrid*/
			doMoodgridRecommendations(user, *itr, myCollection, moodgrid::kMoodgridPresentationType10x10);
		}
	}
	catch (GnError& e)
	{
		std::cout << "ERROR: " << e.ErrorAPI() << "\t" 
				  << std::hex << e.ErrorCode() << "\t" 
