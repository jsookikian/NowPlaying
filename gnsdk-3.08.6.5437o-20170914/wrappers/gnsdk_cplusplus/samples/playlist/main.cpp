 ...
 */
void
display_playlist_results(GnUser& user, playlist::GnPlaylistCollection& collection, playlist::GnPlaylistResults& result)
{
...
do_pdl_generation(GnUser& user, playlist::GnPlaylistCollection& collection)
{
	...
	for (gnsdk_uint32_t stmt_index = 0; stmt_index < count; ++stmt_index)
	{
		...
		playlist::GnPlaylistResults result = collection.GeneratePlaylist(user, pdl_statements[stmt_index], _get_seed_data(user, collection));
...
do_playlist_morelikethis(GnUser& user, playlist::GnPlaylistCollection& collection)
{
	...
	playlist::GnPlaylistResults resultMoreLikeThis = collection.GenerateMoreLikeThis(user, _get_seed_data(user, topColl));
	...
	/*Generating more like this Playlist*/
	playlist::GnPlaylistResults resultCustomMoreLikeThis = collection.GenerateMoreLikeThis(user, _get_seed_data(user, collection));
 ...
 */
void
LoadLocale(GnManager& gnMgr, GnUser& gnUser, GnLookupMode lookupMode)
{
	...
	GnLocale playlistLocale(kLocaleGroupPlaylist, kLanguageEnglish, kRegionDefault, kDescriptorSimplified, gnUser);
    playlistLocale.SetGroupDefault();

    GnLocale musicLocale(kLocaleGroupPlaylist, kLanguageEnglish, kRegionDefault, kDescriptorSimplified, gnUser);
    musicLocale.SetGroupDefault();
}
...
main(int argc, char* argv[])
{
	...
	try
	{
		...
		LoadLocale(gnMgr, user, lookupMode);
		...
		{
			/* Create new collection online if not stored any*/
