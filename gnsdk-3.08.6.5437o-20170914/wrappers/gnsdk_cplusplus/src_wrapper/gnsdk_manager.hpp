...
#define GNSDK_MODULE_LOOKUP_MANAGED    		0x80000
#define GNSDK_MODULE_STORAGE_SQLITE    		0x100000
#define GNSDK_MODULE_STORAGE_QNX			0x200000
#define GNSDK_MODULE_RHYTHM           		0x400000
#define GNSDK_MODULE_RADIOID				0x800000
...
namespace gracenote
{
	...
	public:
		 ...
		 */
		void
		FileSize(gnsdk_uint32_t size_in_kb) throw (GnError);
	...
	public:
		...
		static void
		Initialize() throw (GnError);

		static void
		Shutdown() throw (GnError);

		static void
		ApplyLicense(gnsdk_cstr_t license, GnLicenseInputMode licenseInputMode) throw (GnError);
		...
		void
		SystemEventHandler(GnManagedPtr<IGnSystemEvents*> eventHandler);
	...
	public:
		 ...
		 */
		GnAssetFetch(const GnUser& user, gnsdk_cstr_t url, IGnStatusEvents* pEventHandler = GNSDK_NULL) throw (GnError);
		GnAssetFetch(const GnUser& user, gnsdk_cstr_t url, GnManagedPtr<IGnStatusEvents*> eventHandler) throw (GnError);

		GnAssetFetch(const GnUser& user, gnsdk_cstr_t url, GnLookupMode lookupMode, IGnStatusEvents* pEventHandler = GNSDK_NULL) throw (GnError);
		GnAssetFetch(const GnUser& user, gnsdk_cstr_t url, GnLookupMode lookupMode, GnManagedPtr<IGnStatusEvents*> eventHandler) throw (GnError);

		virtual ~GnAssetFetch();
	...
	private:
		void
		create(const GnUser& user, gnsdk_cstr_t url) throw (GnError);
		void
		create2(const GnUser& user, gnsdk_cstr_t url, GnLookupMode lookupMode) throw (GnError);

		GnManagedPtr<IGnStatusEvents*>	eventHandler_;
		gnsdk_byte_t*					p_data_;
		gnsdk_size_t					size_;
		gnsdk_cstr_t					type_;

		DISALLOW_COPY_AND_ASSIGN(GnAssetFetch);
	};
