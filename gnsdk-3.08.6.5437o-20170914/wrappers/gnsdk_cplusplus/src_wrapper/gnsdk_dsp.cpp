...
/*-----------------------------------------------------------------------------
 *  AudioClassifierStorageLocation
 */
void
GnDsp::AudioClassifierStorageLocation(gnsdk_cstr_t location) throw (GnError)
{
	gnsdk_error_t error;

	error = gnsdk_dsp_storage_location_set(GNSDK_DSP_CLASSIFIER_MODEL_STORAGE_LOCATION, location);
...
GnDspFeatureGenerator::GnDspFeatureGenerator(const GnUser& user, GnDspFeatureType featureType, gnsdk_uint32_t audioSampleRate, gnsdk_uint32_t audioSampleSize, gnsdk_uint32_t audioChannels) throw (GnError)
{
 ...
 */
bool
GnDspFeatureGenerator::FeatureAudioWrite(const gnsdk_byte_t* audioData, gnsdk_size_t audioDataBytes) throw (GnError)
{
	gnsdk_error_t error;
	gnsdk_bool_t  b_complete;

	error = gnsdk_dsp_feature_audio_write(native(), audioData, audioDataBytes, &b_complete);
	if (error) { throw GnError(); }
 ...
 */
void
GnDspFeatureGenerator::FeatureEndOfAudioWrite()  throw (GnError)
{
	gnsdk_error_t error;

	error = gnsdk_dsp_feature_end_of_write(native());
	if (error) { throw GnError(); }
}
 ...
 */
GnDspFeature
GnDspFeatureGenerator::FeatureRetrieve() throw (GnError)
{
	return GnDspFeature(native());
}

GnDspFeatureGenerator::~GnDspFeatureGenerator()
{
}
...
GnDspFeature::GnDspFeature(gnsdk_dsp_feature_handle_t featureHandle) : GnObject<gnsdk_dsp_feature_handle_t>(featureHandle)
{
