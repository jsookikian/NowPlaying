...
moodgrid_result_provider::get_data(gnsdk_uint32_t pos)
{
	...
	{
		error = gnsdk_moodgrid_results_enum(resultHandle_, pos, &media_ident, &group);
		if (!error) 
		{
...
GnMoodgridResult::Count() const throw (GnError)
{
	...
	error = gnsdk_moodgrid_results_count(native(), &count);
...
GnMoodgridProvider::Name() throw (GnError)
{
	...
	error = gnsdk_moodgrid_provider_get_data(native(), GNSDK_MOODGRID_PROVIDER_NAME, &value);
	if (error) { throw GnError(); }
...
GnMoodgridProvider::Type() throw (GnError)
{
	...
	error = gnsdk_moodgrid_provider_get_data(native(), GNSDK_MOODGRID_PROVIDER_TYPE, &value);
	if (error) { throw GnError(); }
...
GnMoodgridPresentation::Moods() const throw (GnError)
{
	...
	gnsdk_error_t error = gnsdk_moodgrid_presentation_get_type(native(), &type);
	if (error) { throw GnError(); }
...
GnMoodgridPresentation::AddFilter(gnsdk_cstr_t uniqueIndentfier, GnMoodgridFilterListType elistType, gnsdk_cstr_t strValueId, GnMoodgridFilterConditionType eConditionType) throw (GnError)
{
	...
	gnsdk_error_t error = gnsdk_moodgrid_presentation_filter_set(native(),
	                                                             uniqueIndentfier,
	                                                             cstrList,
	                                                             strValueId,
	                                                             cstrCondition);
...
GnMoodgridPresentation::RemoveFilter(gnsdk_cstr_t uniqueIndentfier) throw (GnError)
{
	...
	error = gnsdk_moodgrid_presentation_filter_remove(native(), uniqueIndentfier);
	if (error) { throw GnError(); }
}
...
GnMoodgridPresentation::RemoveAllFilters() throw (GnError)
{
	...
	error = gnsdk_moodgrid_presentation_filter_remove_all(native() );
	if (error) { throw GnError(); }
}
...
GnMoodgridPresentation::LayoutType() throw (GnError)
{
	...
	error = gnsdk_moodgrid_presentation_get_type(native(), &eReturnType);
	if (error) { throw GnError(); }
...
GnMoodgridPresentation::MoodName(const GnMoodgridDataPoint& position) throw (GnError)
{
	...
	_convert(coordinate_, LayoutType(), position, calculated);

	gnsdk_error_t error = gnsdk_moodgrid_presentation_get_mood_name(native(), calculated.X, calculated.Y, &name);
	if (error) { throw GnError(); }
...
GnMoodgridPresentation::MoodId(const GnMoodgridDataPoint& position) throw (GnError)
{
	...
	gnsdk_error_t error = gnsdk_moodgrid_presentation_get_mood_id(native(), calculated.X, calculated.Y, &id);
	if (error) { throw GnError(); }
...
GnMoodgridPresentation::FindRecommendations(const GnMoodgridProvider& provider, const GnMoodgridDataPoint& position) throw (GnError)
{
	...
	gnsdk_error_t error = gnsdk_moodgrid_presentation_find_recommendations(native(), provider.native(), calculated.X, calculated.Y, &handle);
	if (error) { throw GnError(); }

	GnMoodgridResult retVal(handle);
	return retVal;
}
...
GnMoodgridPresentation::FindRecommendationsEstimate(const GnMoodgridProvider& provider, const GnMoodgridDataPoint& position) throw (GnError)
{
	...
	error = gnsdk_moodgrid_presentation_find_recommendations_estimate(native(), provider.native(), calculated.X, calculated.Y, &estimate);
	if (error) { throw GnError(); }
...
moodgrid_provider::get_data(gnsdk_uint32_t pos)
{
	...
	return GnMoodgridProvider(provider);
}
...
GnMoodgrid::CreatePresentation(const GnUser& user, GnMoodgridPresentationType type, GnMoodgridCoordinateType coordinate) throw (GnError)
{
	...
	GnMoodgridPresentation result(handle, coordinate);
	return result;
}
