...
_log_package( GnLogPackageType package) 
{
	switch (package) 
	{
		...
		case kLogPackageRadioID				: return GNSDKPKG_RadioID;
...
GnLog::GnLog(gnsdk_cstr_t logFilePath, const GnLogFilters& filters, const GnLogColumns& columns, const GnLogOptions& options, IGnLogEvents *pLoggingDelegate) :
	mLoggingDelegate(pLoggingDelegate)
{
