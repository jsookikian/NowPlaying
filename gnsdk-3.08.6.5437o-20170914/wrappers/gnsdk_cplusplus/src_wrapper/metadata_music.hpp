...
namespace gracenote
{
	...
	namespace metadata
	{
		...
		typedef gn_facade_range_iterator<GnAudioWork, gn_gdo_provider<GnAudioWork> >     audio_work_iterator;
		...
		typedef gn_iterable_container<GnAudioWork, gn_gdo_provider<GnAudioWork> >     audio_work_iterable;
		...
		public:
			...
			AudioWorks() const
			{
				return audio_work_iterable(gn_gdo_provider<GnAudioWork>(*this, GNSDK_GDO_CHILD_AUDIO_WORK),GN_GDO_START);
			}
			...
			Matched() const
			{
				return gnstd::gn_strtobool(StringValue(GNSDK_GDO_VALUE_MATCHED));
			}
			...
			MatchPosition() const
			{
				return IntValue(GNSDK_GDO_VALUE_MATCH_POSITION_MS);
			}
			...
			MatchDuration() const
			{
				return IntValue(GNSDK_GDO_VALUE_MATCH_DURATION_MS);
			}
		   ...
		   /**
			...
			{
				return IntValue(GNSDK_GDO_VALUE_CURRENT_POSITION_MS);
			}
			...
			Duration () const
			{
				return IntValue(GNSDK_GDO_VALUE_DURATION);
			}
			...
			MatchScore() const
			{
				return IntValue(GNSDK_GDO_VALUE_TEXT_MATCH_SCORE);
			}
		...
		public:
			...
			ResultCount() const
			{
				return IntValue(GNSDK_GDO_VALUE_RESPONSE_RESULT_COUNT);
			}
			...
			RangeStart() const
			{
				return IntValue(GNSDK_GDO_VALUE_RESPONSE_RANGE_START);
			}
			...
			RangeEnd() const
			{
				return IntValue(GNSDK_GDO_VALUE_RESPONSE_RANGE_END);
			}
			...
			RangeTotal() const
			{
				return IntValue(GNSDK_GDO_VALUE_RESPONSE_RANGE_TOTAL);
			}
		...
		public:
			...
			DiscInSet() const
			{
				return IntValue(GNSDK_GDO_VALUE_ALBUM_DISC_IN_SET);
			}
			...
			TotalInSet() const
			{
				return IntValue(GNSDK_GDO_VALUE_ALBUM_TOTAL_IN_SET);
			}
			...
			TrackCount() const
			{
				return IntValue(GNSDK_GDO_VALUE_ALBUM_TRACK_COUNT);
			}
			...
			{
				return IntValue(GNSDK_GDO_VALUE_TRACK_MATCHED_NUM, ordinal);
			}
		...
		public:
			GNWRAPPER_ANNOTATE
			GNSDK_CHECKED_CAST(GnResponseDataMatches, GNSDK_GDO_TYPE_RESPONSE_MATCH);
