...
_open_local_db(void)
{
	...
	else
	{
		...
		{
			error = gnsdk_config_value_set(config_handle, GNSDK_CONFIG_LOOKUPDATABASE_ENABLE_MUSICID_CD, GNSDK_VALUE_TRUE);
			if (GNSDK_SUCCESS != error)
			{
				_display_last_error(__LINE__);
				rc = -1;
 ...
 ****************************************************************************/
static int
_set_locale( gnsdk_user_handle_t user_handle, int use_local )
{
	...
	)
{
	...
	if (0 == rc)
	{
		rc = _set_locale(user_handle, use_local);
	}
	...
	)
{
	...
	if (!error)
	{
		error = gnsdk_playlist_storage_enum_collections(0, &collection_name);
	}
	...
	)
{
	...
	for (count_ordinal = 0; count_ordinal < results_count; ++count_ordinal)
	{
		...
		else
		{
			...
			/*	The following illustrates how to get a collection handle from the collection name string in the
			 *	results enum function call.  It ensures that Joined collections as well as non joined collections
			 *	will work with minimal overhead.
			 */
