...
/* Music ID File - album ID notes:
 * If you have many albums where multiple tracks are included, you will be better of using Album ID or Library ID.
 * If you can determine reliably which tracks come from the same album, then grouping by album and feeding to Album ID is best.
 * In RETURN_SINGLE mode, an attempt is made to find albums which contain the highest number of input tracks.
 *   For example, if you have many tracks from a greatest hits album, then the greatest hits will correctly be identified
 *   instead of several individual albums.
 * In RETURN_ALL mode the benefits of using album ID over track ID are lost.
 * Compare the output of this sample with the output of the musicid_file_trackid sample.
 * In online mode, disable the hard coded tag data for both samples so you only use the fingerprint. Compare the results again.
 */
sample_file_t sample_files_1[] = {
    {"RECORDING.wav",                       GNSDK_NULL,  GNSDK_NULL,  GNSDK_NULL}
};

sample_file_t sample_files_2[] = {
		...
		if (use_local != -1)
		{
			...
			if (0 == rc)
			{
				...
				rc = _do_sample_albumid(
                    ...
                    sample_files_1,
                    sizeof(sample_files_1)/sizeof(sample_file_t)
				);
                if (0 == rc)
                {
                    rc = _do_sample_albumid(
                        ...
                        GNSDK_MUSICIDFILE_QUERY_FLAG_RETURN_SINGLE | midf_query_flag,
                        sample_files_2,
                        sizeof(sample_files_2)/sizeof(sample_file_t)
                    );
                }
	...
	)
{
    ...
    for (index = 0; index < count && rc == 0; index++)
    {
	...
	)
{
	...
	if (0 == rc)
	{
		rc = _set_query_data(query_handle, sample_files, count, use_local);
	}
	...
	if (0 == rc)
	{
		error = gnsdk_musicidfile_query_fileinfo_count(query_handle, &result_count);
		if (GNSDK_SUCCESS != error)
		{
			_display_last_error(__LINE__);
			rc = -1;
		...
		else
		{
			...
			for (index = 0; index < result_count; index++)
			{
				...
				printf("*File %2d of %2d*", index + 1, result_count);
