...
_open_local_db(void)
{
	...
	else
	{
		...
		{
			error = gnsdk_config_value_set(config_handle, GNSDK_CONFIG_LOOKUPDATABASE_ENABLE_MUSICID, GNSDK_VALUE_TRUE);
			if (GNSDK_SUCCESS != error)
			{
				_display_last_error(__LINE__);
				rc = -1;
...
/****************************************************************************
 *
 *    _NAVIGATE_NAME_TLS_GDO
 *
 *	Description: This function demonstrates	how	to navigate	a 'Name	TLS'
 *	GDO	retrieved from a parent	GDO. It	takes a	parent GDO as its argument.
 *
 ****************************************************************************/
static void
...
{
	gnsdk_error_t      error             = GNSDK_SUCCESS;
	gnsdk_gdo_handle_t name_tls_gdo		 = GNSDK_NULL;
	gnsdk_uint32_t     child_count       = 0;
	gnsdk_char_t       tab_string[1024]  = "";
	...
	/* Get the 'Name TLS' GDO from	the	contributor	GDO	*/
	error = gnsdk_manager_gdo_child_count(parent_gdo, GNSDK_GDO_CHILD_NAME_TLS, &child_count);

	if (child_count > 0)
	{
		error = gnsdk_manager_gdo_child_get(parent_gdo, GNSDK_GDO_CHILD_NAME_TLS, 1, &name_tls_gdo);
	...
	/* Navigate	Name TLS GDO */
	printf("%sName Three-Line-Solution:\n", tab_string);

	/* Retrieve	the	display	value string from the 'Name	TLS' GDO */
	_display_gdo_value(name_tls_gdo, GNSDK_GDO_VALUE_DISPLAY, "Display", tab_index);

	gnsdk_manager_gdo_release(name_tls_gdo);
	...
	)
{
	...
	/* Get the 'Name TLS' GDO from	the	contributor	GDO	*/
	_navigate_name_tls_gdo(contributor_gdo, tab_index);
...
/****************************************************************************
 *
 *    _NAVIGATE_AUDIO_WORK_GDO
 *
 *	Description: This function demonstrates	how	to navigate	a 'Audio Work'
 *		GDO	retrieved from a parent	GDO.
 *
 ****************************************************************************/
static void 
_navigate_audio_work_gdo(
	gnsdk_gdo_handle_t parent_gdo,
	gnsdk_uint32_t	   audio_work_ordinal,
	gnsdk_uint32_t	   tab_index
	) 
{
	gnsdk_error_t      error              =   GNSDK_SUCCESS;
	gnsdk_gdo_handle_t audio_work_gdo	  =		GNSDK_NULL;
	gnsdk_char_t       tab_string[1024]   =   "";

	_create_tab_string(tab_index, tab_string);
	tab_index += 1;

	/* Get Audio Work gdo from parent gdo */
	error = gnsdk_manager_gdo_child_get(parent_gdo, GNSDK_GDO_CHILD_AUDIO_WORK, audio_work_ordinal, &audio_work_gdo);
	...
	/* Navigate	audio work GDO */
	printf("%sAudio Work:\n", tab_string);

	/* Display the TUI for this audio work GDO */
	_display_gdo_value(audio_work_gdo, GNSDK_GDO_VALUE_TUI, "TUI", tab_index);

	/* Navigate	the	'Title Official' GDO from the track	GDO	*/
	_navigate_title_official_gdo(audio_work_gdo, tab_index);	

	/* Navigate	the	credit GDO from	this aduio work GDO 
	_navigate_credit_gdo(audio_work_gdo, GNSDK_GDO_CHILD_CREDIT, tab_index);*/

	/* Display the TUI for this audio work GDO */
	_display_gdo_value(audio_work_gdo, GNSDK_GDO_VALUE_COMPOSITION_FORM, "Composition Form", tab_index);

	gnsdk_manager_gdo_release(audio_work_gdo);
	...
	)
{
	...
	gnsdk_uint32_t	   audio_work_count = 0;
	gnsdk_uint32_t	   audio_work_ordinal = 1;
	gnsdk_char_t       tab_string[1024] = "";
	...
	/* Get the audio work GDO child count from the track GDO	*/
	error = gnsdk_manager_gdo_child_count(track_gdo, GNSDK_GDO_CHILD_AUDIO_WORK, &audio_work_count);
	...
	/* Navigate	each audio work GDO from the album GDO for the audio work count */
	for (audio_work_ordinal = 1; audio_work_ordinal <= audio_work_count; audio_work_ordinal++)
	{
		_navigate_audio_work_gdo(track_gdo, audio_work_ordinal, tab_index);
	}
