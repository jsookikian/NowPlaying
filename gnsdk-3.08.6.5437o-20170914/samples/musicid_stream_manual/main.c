...
_open_local_db(void)
{
	...
	{
		error = gnsdk_config_value_set(config_handle, GNSDK_CONFIG_LOOKUPDATABASE_ALL_LOCATION, SAMPLE_LOOKUPDATABASE_LOCATION);
		if (GNSDK_SUCCESS != error)
		{
			_display_last_error(__LINE__);
			rc = -1;
		...
		else
		{
			...
			else
			{
				...
				error = gnsdk_lookupdatabase_open(SAMPLE_LOOKUPDATABASE_ID, config_handle);
				if (GNSDK_SUCCESS != error)
				{
					_display_last_error(__LINE__);
					rc = -1;
				...
				else
				{
					...
					_display_local_db_info( SAMPLE_LOOKUPDATABASE_ID );
				}
			}
		}
...
_set_locale( gnsdk_user_handle_t user_handle )
{
	...
	error = gnsdk_manager_storage_location_set( GNSDK_MANAGER_STORAGE_LISTS, SAMPLE_LOOKUPDATABASE_LOCATION );
	if (GNSDK_SUCCESS == error)
	{
	...
	)
{
	...
	if (use_local)
	{
		...
		if (0 == rc)
		{
			...
			{
				error = gnsdk_lookup_localstream_storage_location_set(SAMPLE_LOOKUPDATABASE_LOCATION);
				if (GNSDK_SUCCESS != error)
				{	
					_display_last_error(__LINE__);
					rc = -1;
	...
	)
{
	...
	/* This app doesn't allow a user to cancel identification but you could use this parameter
