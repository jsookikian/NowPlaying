	...
	)
{
	...
	if (GNSDK_SUCCESS == error)
	{
		error = gnsdk_link_query_option_set(
					...
					GNSDK_CONTENT_DATA_SIZE_VALUE_170
					);
		if (GNSDK_SUCCESS != error)
		{
			_display_last_error(__LINE__);
	...
	)
{
	...
	if (error == GNSDK_SUCCESS)
	{
		/* we were able to identify the input genre string */
		got_match = 1;
				...
				if (count > 0)
				{
					...
					else
					{
						...
						else
						{
							...
							rc = _perform_image_fetch(match_gdo, gdo_type, user_handle);
						}
	...
	if (rc == 0)
	{
		...
		for (file_index = 0; file_index < sizeof(_mp3_folder)/sizeof(mp3_file_t); file_index++)
		{
			...
			if (0 == got_match)
			{
				...
				{
					_find_genre_image(user_handle, mp3_file->genre_tag, genre_list_handle, &got_match);
				}
			}
