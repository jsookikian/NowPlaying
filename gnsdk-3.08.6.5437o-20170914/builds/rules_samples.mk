...
ifneq "$(wildcard ../../builds )" ""
    ...
    ifneq "$(wildcard $(build_dir)/../version )" ""
        sdk_version = $(build_dir)/../version
    else
...
ifneq "$(wildcard ../../builds )" ""
    ...
    ifneq "$(wildcard $(build_dir)/../utilities )" ""
...
ifneq "$(wildcard ../../builds )" ""
    ...
    ifneq "$(wildcard $(GNSDK_INSTALL_DIR)/include )" ""
        GNSDK_INCLUDE = $(GNSDK_INSTALL_DIR)/include
    else
...
ifeq ($(STATIC),static)
    ifneq "$(wildcard ../../builds )" ""
        ...
        ifneq "$(wildcard $(GNSDK_INSTALL_DIR)/lib_static )" ""
            GNSDK_LIBPATH = $(GNSDK_INSTALL_DIR)/lib_static/$(IMPLDIR)_$(IMPLARCH)
        else
    ...
    ifneq "$(wildcard ../../builds )" ""
        ...
        ifneq "$(wildcard  $(GNSDK_INSTALL_DIR)/lib )" ""
            GNSDK_LIBPATH = $(GNSDK_INSTALL_DIR)/lib/$(IMPLDIR)_$(IMPLARCH)
        else
...
ifneq ($(use_gnsdk_biglib),)
GNSDK_LINKDEPS = $(subst _,,$(subst %,,$(GNSDK_LINKPATTERN)))
GNSDK_LIBDEPS = $(subst _,, $(subst %,,$(GNSDK_LIBPATTERN))) 
else
...
ifeq ($(platformlibs),)
    ifeq ($(IMPLDIR), win)
        platformlibs += advapi32.lib ws2_32.lib iphlpapi.lib
...
#
ifneq ($(extra_libs),)
    EXTRA_LIBS_DIR := $(extra_libs)/$(IMPLDIR)/$(IMPLARCH)
    EXTRA_LIBS := $(notdir $(wildcard $(EXTRA_LIBS_DIR)/*))
endif
...
#
# link target library with created output and dependent libs
#
...
make_target_requisites =  $(objects_a) $(call mkdir_target,$1) $(addprefix $(OUTPUT_DIR)/$1/,$(objects)) $(GNSDK_LINKDEPS) $(EXTRA_LIBS) | showinfo
